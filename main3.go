...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier * float64(w.Weight)) + (math.Pow((w.meanSpeed()*KmHInMsec), 2) / (w.Height / CmInM) * CaloriesSpeedHeightMultiplier * float64(w.Weight))) * w.Duration.Hours()
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	return (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * float64(s.Weight) * float64(s.Weight) * s.Duration.Hours()
}
